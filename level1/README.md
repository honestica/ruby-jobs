...
La consigne principale pour ce niveau est donc d'écrire l'application qui va générer `output.json` (on s'intéresse aux résultats plus qu'à la fore du document) à partir de `data.json`.
